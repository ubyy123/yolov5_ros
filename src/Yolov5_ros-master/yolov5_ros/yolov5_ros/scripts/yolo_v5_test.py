...
from message.msg import MultiPoint
class Yolo_Dect:
    def __init__(self):
        ...
        #position 
        self.center_pub = rospy.Publisher('/yolov5/ObjectCenters', MultiPoint, queue_size=1)
    ...
    def dectshow(self, org_img, boxs, height, width):
        img = org_img.copy()
        classes_num = {}
        multi_point_msg = MultiPoint()
        multi_point_msg.points = []
        multi_point_msg.points1 = []
        multi_point_msg.strings = []
        count = 0
        for i in boxs:
            count += 1
        ...
        for box in boxs:
            ...
            multi_point_msg.points.append(Point(box[0],box[1],0))
            multi_point_msg.points1.append(Point(box[2],box[3],0))
            multi_point_msg.strings.append(box[-1])
        ...
        self.center_pub.publish(multi_point_msg)
